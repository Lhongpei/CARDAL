...
extern "C" void cardal_sigint_handler(int) { cardal_request_cancel(); }
...
py::array_t<double> borrow_as_numpy(const double *data, py::ssize_t n,
                                    py::object owner) {
  if (data == nullptr || n <= 0)
  ...
  return py::array_t<double>({n},               // shape
                             {sizeof(double)},  // strides
                             data,              // pointer
                             std::move(owner)); // base
}

py::array_t<int> borrow_int_as_numpy(const int *data, py::ssize_t n,
                                     py::object owner) {
  if (data == nullptr || n <= 0)
    return py::array_t<int>({0});
  return py::array_t<int>({n}, {sizeof(int)}, data, std::move(owner));
}
...
cardal_params params_from_dict(const py::dict &d) {
  cardal_params p;
  ...
  for (auto item : d) {
    ...
    if (key == "eps_optimal_relative")
      p.eps_optimal_relative = v.cast<double>();
    else if (key == "eps_primal_relative")
      p.eps_primal_relative = v.cast<double>();
    else if (key == "eps_dual_relative")
      p.eps_dual_relative = v.cast<double>();
    else if (key == "time_sec_limit")
      p.time_sec_limit = v.cast<double>();
    else if (key == "iteration_limit")
      p.iteration_limit = v.cast<int>();
    else if (key == "initial_rank")
      p.initial_rank = v.cast<int>();
    else if (key == "max_rank")
      p.max_rank = v.cast<int>();
      ...
      if (py::isinstance<py::str>(v)) {
        ...
        if (mode == "random")
          ...
          throw py::value_error("augmentation_mode must be 'random', 'qp', "
                                "'closed-form', or 'sdp'");
      } else {
    ...
    } else if (key == "lbfgs_history_size")
      p.lbfgs_history_size = v.cast<int>();
    else if (key == "penalty_factor")
      p.penalty_factor = v.cast<double>();
    else if (key == "initial_penalty_coef")
      p.initial_penalty_coef = v.cast<double>();
    else if (key == "max_penalty_coef")
      p.max_penalty_coef = v.cast<double>();
    else if (key == "inner_iterations_limit")
      p.inner_iterations_limit = v.cast<long>();
    else if (key == "l_inf_ruiz_iterations") {
      p.l_inf_ruiz_iterations = v.cast<int>();
      if (p.l_inf_ruiz_iterations < 0)
        throw py::value_error("l_inf_ruiz_iterations must be nonnegative");
      ...
      if (p.pock_chambolle_alpha < 0.0)
        throw py::value_error("pock_chambolle_alpha must be nonnegative");
...
struct CoreProblem {
  ...
  CoreProblem(CoreProblem &&other) noexcept : raw(other.raw) {
    other.raw = nullptr;
  }
  CoreProblem &operator=(CoreProblem &&other) noexcept {
    if (this != &other) {
      if (raw)
        cardal_problem_free(raw);
      raw = other.raw;
      other.raw = nullptr;
    }
    return *this;
...
struct CoreResult {
  ...
  CoreResult(CoreResult &&other) noexcept : raw(other.raw) {
    other.raw = nullptr;
  }
...
PYBIND11_MODULE(_core, m) {
  ...
  py::class_<CoreProblem>(m, "Problem")
      .def_property_readonly("num_constraints",
                             [](CoreProblem &self) {
                               return cardal_problem_num_constraints(self.raw);
                             })
      ...
      .def_property_readonly("num_variables",
                             [](CoreProblem &self) {
                               return cardal_problem_num_variables(self.raw);
                             })
      .def_property_readonly(
          "lp_dim",
      ...
      .def_property_readonly("block_dims", [](CoreProblem &self) {
        int n = cardal_problem_num_cones(self.raw);
        std::vector<int> dims(std::max(n, 1));
        if (n > 0)
          cardal_problem_get_block_dims(self.raw, dims.data());
        dims.resize(n);
        return dims;
      });
  ...
  py::class_<CoreResult>(m, "Result")
      ...
      .def_property_readonly("primal_objective",
                             [](CoreResult &self) {
                               return cardal_result_primal_objective(self.raw);
                             })
      .def_property_readonly("dual_objective",
                             [](CoreResult &self) {
                               return cardal_result_dual_objective(self.raw);
                             })
      .def_property_readonly("objective_gap",
                             [](CoreResult &self) {
                               return cardal_result_objective_gap(self.raw);
                             })
      .def_property_readonly("rel_primal_residual",
                             [](CoreResult &self) {
                               return cardal_result_rel_primal_residual(
                                   self.raw);
                             })
      .def_property_readonly("rel_dual_residual",
                             [](CoreResult &self) {
                               return cardal_result_rel_dual_residual(self.raw);
                             })
      .def_property_readonly("rel_objective_gap",
                             [](CoreResult &self) {
                               return cardal_result_rel_objective_gap(self.raw);
                             })
      .def_property_readonly(
          "runtime_sec",
      ...
      .def_property_readonly("num_variables",
                             [](CoreResult &self) {
                               return cardal_result_num_variables(self.raw);
                             })
      .def_property_readonly("num_constraints",
                             [](CoreResult &self) {
                               return cardal_result_num_constraints(self.raw);
                             })
      .def_property_readonly(
          "total_rank",
  ...
  m.def(
      ...
      []() {
        ...
        d["pock_chambolle_rescaling"] = p.pock_chambolle_rescaling != 0;
        d["pock_chambolle_alpha"] = p.pock_chambolle_alpha;
        d["bound_objective_rescaling"] = p.bound_objective_rescaling != 0;
        d["psd_scale_mode"] = p.psd_scale_mode == CARDAL_PSD_SCALE_PER_CONE
                                  ? "per-cone"
                                  : "per-element";
        d["verbose"] = p.verbose;
        return d;
  ...
  m.def(
      "build_problem",
      [](int num_constraints, int num_cones, int lp_dim, int free_dim,
         ...
         // Signed low-rank PSD terms
         py::array_t<int, py::array::c_style | py::array::forcecast>
             c_lr_cone_ind,
         py::array_t<int, py::array::c_style | py::array::forcecast> c_lr_rank,
         ...
         py::array_t<int, py::array::c_style | py::array::forcecast>
             a_lr_cone_ind,
         py::array_t<int, py::array::c_style | py::array::forcecast> a_lr_rank,
         py::array_t<double, py::array::c_style | py::array::forcecast>
             a_lr_factors,
         py::array_t<double, py::array::c_style | py::array::forcecast>
             a_lr_weights,
        ...
        auto check_size = [](const char *name, py::ssize_t got,
                             py::ssize_t want) {
          if (got != want) {
        ...
        if (num_constraints < 0 || num_cones < 0 || lp_dim < 0 || free_dim < 0)
          throw py::value_error(
        ...
        const py::ssize_t n_c_lr = c_lr_rank.size();
        check_size("c_lr_cone_ind", c_lr_cone_ind.size(), n_c_lr);
        const py::ssize_t n_a_lr = a_lr_rank.size();
        check_size("a_lr_constr_ind", a_lr_constr_ind.size(), n_a_lr);
        check_size("a_lr_cone_ind", a_lr_cone_ind.size(), n_a_lr);
        py::ssize_t expected_c_lr_factors = 0;
        py::ssize_t expected_c_lr_weights = 0;
        for (py::ssize_t t = 0; t < n_c_lr; t++) {
          int cone = c_lr_cone_ind.data()[t];
          int rank = c_lr_rank.data()[t];
          if (cone < 0 || cone >= num_cones || rank <= 0)
            throw py::value_error("invalid C low-rank cone or rank");
          expected_c_lr_factors += (py::ssize_t)blk_dims.data()[cone] * rank;
          expected_c_lr_weights += rank;
        }
        check_size("c_lr_factors", c_lr_factors.size(), expected_c_lr_factors);
        check_size("c_lr_weights", c_lr_weights.size(), expected_c_lr_weights);
        py::ssize_t expected_a_lr_factors = 0;
        py::ssize_t expected_a_lr_weights = 0;
        for (py::ssize_t t = 0; t < n_a_lr; t++) {
          int constr = a_lr_constr_ind.data()[t];
          int cone = a_lr_cone_ind.data()[t];
          int rank = a_lr_rank.data()[t];
          if (constr < 0 || constr >= num_constraints || cone < 0 ||
              cone >= num_cones || rank <= 0)
            throw py::value_error(
                "invalid A low-rank constraint, cone, or rank");
          expected_a_lr_factors += (py::ssize_t)blk_dims.data()[cone] * rank;
          expected_a_lr_weights += rank;
        }
        check_size("a_lr_factors", a_lr_factors.size(), expected_a_lr_factors);
        check_size("a_lr_weights", a_lr_weights.size(), expected_a_lr_weights);
        ...
        d.num_c_lr_terms = (int)n_c_lr;
        d.c_lr_cone_ind = n_c_lr > 0 ? c_lr_cone_ind.data() : nullptr;
        d.c_lr_rank = n_c_lr > 0 ? c_lr_rank.data() : nullptr;
        d.c_lr_factors =
            expected_c_lr_factors > 0 ? c_lr_factors.data() : nullptr;
        d.c_lr_weights =
            expected_c_lr_weights > 0 ? c_lr_weights.data() : nullptr;
        d.num_a_lr_terms = (int)n_a_lr;
        d.a_lr_constr_ind = n_a_lr > 0 ? a_lr_constr_ind.data() : nullptr;
        d.a_lr_cone_ind = n_a_lr > 0 ? a_lr_cone_ind.data() : nullptr;
        d.a_lr_rank = n_a_lr > 0 ? a_lr_rank.data() : nullptr;
        d.a_lr_factors =
            expected_a_lr_factors > 0 ? a_lr_factors.data() : nullptr;
        d.a_lr_weights =
            expected_a_lr_weights > 0 ? a_lr_weights.data() : nullptr;
        ...
        d.free_constr_ind = nnz_free > 0 ? free_constr_ind.data() : nullptr;
        ...
        if (raw == nullptr) {
          ...
          case CARDAL_E_NULL_ARG:
            throw py::value_error(
                "invalid problem data (null arg or empty shape)");
          default:
      ...
      py::arg("num_constraints"), py::arg("num_cones"), py::arg("lp_dim"),
      py::arg("free_dim"), py::arg("blk_dims"), py::arg("c_cone_ind"),
      py::arg("c_row_ind"), py::arg("c_col_ind"), py::arg("c_val"),
      py::arg("a_constr_ind"), py::arg("a_cone_ind"), py::arg("a_row_ind"),
      py::arg("a_col_ind"), py::arg("a_val"), py::arg("c_lr_cone_ind"),
      py::arg("c_lr_rank"), py::arg("c_lr_factors"), py::arg("c_lr_weights"),
      py::arg("a_lr_constr_ind"), py::arg("a_lr_cone_ind"),
      py::arg("a_lr_rank"), py::arg("a_lr_factors"), py::arg("a_lr_weights"),
      py::arg("lp_obj"), py::arg("lp_constr_ind"), py::arg("lp_col_ind"),
      py::arg("lp_val"), py::arg("free_obj"), py::arg("free_constr_ind"),
      py::arg("free_col_ind"), py::arg("free_val"), py::arg("b"),
  ...
  m.def(
      ...
      py::arg("problem"), py::arg("params") = py::dict(),
  ...
  m.def(
      "clear_cancel", &cardal_clear_cancel,
      "Clear the cancellation flag (called automatically before each solve).");
  m.def(
      "cancel_requested", []() { return cardal_cancel_requested() != 0; },
